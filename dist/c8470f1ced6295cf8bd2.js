...
var warmUpQuestion = document.getElementsByClassName('question-type')[0];
...
function fillAnswerButtonsWithOptions() {
  var answerButtonsArray = Array.from(document.getElementsByClassName('answer-option'));
  var questionTypes = Array.from(document.getElementsByClassName('question-type'));
  questionTypes.forEach(function (el, i) {
    if (el.classList.contains('active')) {
      currQuesPoolNum = i;
    }
    ;
  });
  for (var i = 0; i <= birdsData[currQuesPoolNum].length - 1; i++) {
    answerButtonsArray[i].textContent = birdsData[currQuesPoolNum][i].name;
  }
}
fillAnswerButtonsWithOptions();
function getQuestion() {
  var currentBirdArray = birdsData[currQuesPoolNum];
  var randomQuestion = randomIntFromInterval(0, 5);
...
// player settings
var soundSource = currQues.audio;
...
sound.addEventListener('timeupdate', function () {
  var currTime = parseInt(sound.currentTime, 10);
...
function getRightAnswer(currQuesPoolNum, currQues) {
  var answerOptions = Array.from(document.getElementsByClassName('answer-button'));
  var flag = false;
  function colorIndicator(e) {
    if (!flag) {
      e.target.closest('button').classList.add('active');
      var answerIndicatior = e.target.closest('button').getElementsByClassName('round')[0];
...
function fillInfoBlock() {
  if (rightAnswer) {
    ...
    var namesblock = document.getElementsByClassName('bird-names')[0];
    var mainName = document.getElementsByClassName('main-name')[0];
    var player = document.getElementById('small-player');
    var latinName = document.getElementsByClassName('latin-name')[0];
    var birdInfo = document.getElementsByClassName('bird-info')[0];
